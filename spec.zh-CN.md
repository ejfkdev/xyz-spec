...
- `min`/`max`/`len` —— 字符串与切片比较长度；数值类型比较值；浮点比较
  其截断后的整数形式。
- `gt`/`gte`/`lt`/`lte` —— 仅数值类型；其他类型一律不通过该规则。
...
`-v/--version` 出现在命令行任意位置时打印 `<bin> version <version>` 并
退出 0；`--json` 出现在任意位置时把结果渲染切换为 JSON；
...
按 §4.6）、form 字段（body + query）。被 §4.1 排除的字段按键为语言字段名
接收 header 值。
